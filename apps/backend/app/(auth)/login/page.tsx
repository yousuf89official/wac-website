...
/**
 * Validate a ?returnTo= target before redirecting to it. Prevents open-redirect:
 * only relative paths or our own hosts (localhost + *.wearecollaborative.net) are
 * allowed. Used to bounce customers back to the surface that sent them here —
 * e.g. the CI subdomain (intelligence.wearecollaborative.net).
 */
function safeReturnTo(raw: string | null): string | null {
    if (!raw) return null;
    if (raw.startsWith('/') && !raw.startsWith('//')) return raw; // relative path
    try {
        const url = new URL(raw);
        const host = url.hostname;
        const allowed =
            host === 'localhost' ||
            host === 'wearecollaborative.net' ||
            host.endsWith('.wearecollaborative.net');
        ...
        <div className="mx-auto grid min-h-screen max-w-[1440px] grid-cols-1 gap-x-6 px-6 pt-28 pb-16 md:grid-cols-12 md:px-12 md:pt-32 lg:px-20">
                ...
                >
                    <div className="rule-hairline pt-6">
                        <p className="font-serif text-lg leading-relaxed text-foreground-soft">
                            &ldquo;The work continues. Pick up where you left off — courses, orders,
                            and the community are waiting.&rdquo;
                        </p>
            ...
            >
                <div className="md:sticky md:top-32">
                    <p className="eyebrow mb-8">Sign in to your portal</p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        <div>
                            <div className="flex items-center justify-between">
                                <label className="font-mono text-2xs font-medium uppercase tracking-widest text-foreground-soft">
                                    Password
                                </label>
                                <Link
                                    href="/forgot-password"
                                    className="font-mono text-2xs uppercase tracking-widest text-foreground-soft transition-colors hover:text-primary"
                                >
                                    Forgot?
                                </Link>
                            </div>
