...
import type { IndexTrendPoint } from '@/types/lens';
import { seriesColor, CHART_GRID, CHART_AXIS_TICK } from '@/lib/chart-theme';
    ...
    <div className="card p-4">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={filteredTrend} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
          ...
          <XAxis dataKey="displayDate" tick={{ ...CHART_AXIS_TICK, fontSize: 10 }} />
          <YAxis tick={CHART_AXIS_TICK} domain={[20, 90]} />
          <Tooltip content={<CustomTooltip />} />
          <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: '11px' }} />
          <Line type="monotone" dataKey="NSS" stroke={seriesColor(0)} strokeWidth={2} dot={false} />
          <Line type="monotone" dataKey="SI" stroke={seriesColor(1)} strokeWidth={2} dot={false} />
          <Line type="monotone" dataKey="EMSS" stroke={seriesColor(2)} strokeWidth={2} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
