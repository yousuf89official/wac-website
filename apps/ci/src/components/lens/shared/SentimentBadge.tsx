...
import type { Sentiment } from '@/types/lens';
...
const config: Record<Sentiment, { bg: string; text: string; label: string }> = {
  ...
  neutral: { bg: 'bg-card', text: 'text-foreground-soft', label: 'Neutral' },
  ...
  return (
    <span className={`inline-flex items-center rounded font-medium ${c.bg} ${c.text} ${
      ...
      <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
        sentiment === 'positive' ? 'bg-green-400' : sentiment === 'negative' ? 'bg-red-400' : 'bg-foreground-soft'
      }`} />
      {c.label}
    </span>
  );
